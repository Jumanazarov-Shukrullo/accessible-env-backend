...
def run_linting():
    ...
    # Run flake8
    cmd = [sys.executable, "-m", "flake8", "app/", "tests/", "--max-line-length=120", "--ignore=E203,W503"]
...
def run_security_tests():
    ...
    cmd = [sys.executable, "-m", "bandit", "-r", "app/", "-f", "json", "-o", "bandit-report.json"]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run comprehensive tests for the platform")
    ...
    parser.add_argument("--services-only", action="store_true", help="Run only service tests")
    parser.add_argument("--routes-only", action="store_true", help="Run only route tests")

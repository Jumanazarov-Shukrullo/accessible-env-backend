...
class TestRunner:
    ...
    def test_backend_lint(self):
        ...
        commands = [
            ("flake8 --max-line-length=100 --exclude=.venv,migrations .", "Flake8 linting"),
            ("black --check --diff .", "Black formatting check")
        ]
        
        all_passed = True
        for cmd, desc in commands:
            success, _ = self.run_command(cmd, cwd=self.backend_dir.parent, description=desc)
            if not success:
                all_passed = False
    ...
    def test_frontend_lint(self):
        ...
        all_passed = True
        for cmd, desc in commands:
            success, _ = self.run_command(cmd, cwd=self.frontend_dir, description=desc)
            if not success:
                # Frontend linting might not be configured, so we'll warn but not fail
                self.log(f"Frontend {desc} failed - might not be configured", "WARNING")

...
from sqlalchemy import DECIMAL, JSON, DateTime, Enum, Float, ForeignKey, Integer, String, Text, Boolean
...
class Location(Base):
    ...
    city_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("city.city_id"))
    ...
    # Relationships
    details = relationship("LocationDetails", back_populates="location", uselist=False, cascade="all, delete-orphan")
    stats = relationship("LocationStats", back_populates="location", uselist=False, cascade="all, delete-orphan")
...
class LocationDetails(Base):
    ...
    location_id: Mapped[str] = mapped_column(String, ForeignKey("locations.location_id", ondelete="CASCADE"), primary_key=True)
    ...
    created_at: Mapped[dt.datetime] = mapped_column(DateTime(timezone=True), default=dt.datetime.now)
    updated_at: Mapped[dt.datetime] = mapped_column(DateTime(timezone=True), default=dt.datetime.now, onupdate=dt.datetime.now)
...
class LocationStats(Base):
    ...
    last_assessment_date: Mapped[dt.datetime | None] = mapped_column(DateTime(timezone=True))
    updated_at: Mapped[dt.datetime] = mapped_column(DateTime(timezone=True), default=dt.datetime.now, onupdate=dt.datetime.now)

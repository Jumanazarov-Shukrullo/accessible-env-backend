...
from app.models.assessment_model import AssessmentSet, SetCriteria, AccessibilityCriteria
...
class AssessmentSetRepository(SQLAlchemyRepository[AssessmentSet, int]):
    ...
    def active(self):
        return self.db.scalars(select(AssessmentSet).where(AssessmentSet.is_active)).all()

    def get_criteria(self, set_id: int):
        # Query criteria through the junction table
        criteria = (
            self.db.query(AccessibilityCriteria)
            .join(SetCriteria, SetCriteria.criterion_id == AccessibilityCriteria.criterion_id)

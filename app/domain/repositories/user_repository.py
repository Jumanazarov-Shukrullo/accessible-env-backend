...
class UserRepository(SQLAlchemyRepository[User, str], IUserRepository):
    ...
    def create(self, user_data: dict) -> User:
        ...
        core_fields = {"username", "email", "role_id", "is_active", "email_verified"}
        profile_fields = {"first_name", "surname", "middle_name", "full_name", "phone_number", "profile_picture", "language_preference"}
        security_fields = {"last_login_at", "failed_login_attempts", "password_reset_token", "password_reset_token_expires", "two_factor_enabled", "last_login_ip", "password_changed_at"}
        
        # Extract password and hash it
        plain_password = user_data.pop("password", None) or user_data.pop("password_hash", None)
        if plain_password is None:
            raise ValueError("Password is required")
        
        # Prepare core user data
        core_data = {k: v for k, v in user_data.items() if k in core_fields}
        core_data["password_hash"] = SecurityManager.get_password_hash(plain_password)
        ...
        profile_data = {k: v for k, v in user_data.items() if k in profile_fields and v is not None}
        ...
        security_data = {k: v for k, v in user_data.items() if k in security_fields and v is not None}
    ...
    def get_by_password_reset_token(self, token: str) -> Optional[User]:
        ...
        return (
            ...
            .filter(UserSecurity.password_reset_token_expires > datetime.now(timezone.utc))
            .first()
        )
    ...
    @cache.cacheable(lambda self, fields, limit=20, offset=0: f"users:fields:{','.join(fields)}:{limit}:{offset}", ttl=300)
    def get_specific_fields(self, fields: List[str], limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        """Fetch only specific fields of user records, joining profile and security tables as needed"""
        logger.info(f"Fetching user fields {fields} with limit {limit}, offset {offset}")
        ...
        profile_fields = [f for f in fields if hasattr(UserProfile, f) and f != 'user_id']
        security_fields = [f for f in fields if hasattr(UserSecurity, f) and f != 'user_id']
        ...
        if security_fields:
            query = query.outerjoin(UserSecurity, User.user_id == UserSecurity.user_id)
        ...
        logger.info(f"User query completed in {(end_time - start_time).total_seconds():.2f} seconds, found {len(users_data)} users")
        return users_data
    
    @cache.cacheable(lambda self, role_id, fields, limit=20, offset=0: f"users:role:{role_id}:fields:{','.join(fields)}:{limit}:{offset}", ttl=300)
    def get_users_by_role(self, role_id: int, fields: List[str], limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        end_time = datetime.now()
        logger.info(f"Role-based user query completed in {(end_time - start_time).total_seconds():.2f} seconds, found {len(users_data)} users")
        return users_data
    
    @cache.cacheable(lambda self, search_term, fields, limit=20, offset=0: f"users:search:{search_term}:fields:{','.join(fields)}:{limit}:{offset}", ttl=300)
    def search_users(self, search_term: str, fields: List[str], limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        query = self.db.query(User).outerjoin(UserProfile, User.user_id == UserProfile.user_id)
        ...
        users = query.filter(or_(*search_filters)).order_by(User.created_at.desc()).offset(offset).limit(limit).all()
        ...
        end_time = datetime.now()
        logger.info(f"Search users query completed in {(end_time - start_time).total_seconds():.2f} seconds, found {len(users_data)} users")
        return users_data
    
    @cache.cacheable(lambda self, role_id=None: f"users:count:role:{role_id}", ttl=300)
    ...
    @cache.cacheable(lambda self, search_term: f"users:count:search:{search_term}", ttl=300)
    def count_with_search(self, search_term: str) -> int:
        ...
        query = self.db.query(func.count(User.user_id.distinct())).outerjoin(UserProfile, User.user_id == UserProfile.user_id)
    ...
    @cache.cacheable(lambda self, limit=20, offset=0: f"users:minimal:{limit}:{offset}", ttl=300)
    def get_minimal_users_paginated(self, limit: int, offset: int) -> Tuple[List[Dict[str, Any]], int]:
        """Get paginated users with all required fields for UserResponse model"""
        logger.info(f"Fetching minimal users paginated with limit {limit}, offset {offset}")
        start_time = datetime.now()
        
        required_fields = [
            "user_id", "username", "email", "first_name", "surname", "middle_name", 
            "role_id", "is_active", "email_verified", "created_at", "profile_picture", 
        ...
        end_time = datetime.now()
        logger.info(f"Minimal users query completed in {(end_time - start_time).total_seconds():.2f} seconds")
        return users, total

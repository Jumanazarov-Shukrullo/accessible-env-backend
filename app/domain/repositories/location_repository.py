...
class LocationRepository(SQLAlchemyRepository[Location, UUID]):
    ...
    def get_full(self, location_id: UUID) -> Location | None:
        ...
        logger.info(
            f"Repository: Fetching location {location_id} with related data"
        )
        ...
        stmt = (
            ...
            .options(
                ...
                selectinload(Location.inspectors).selectinload(
        ...
        end_time = time.time()
        logger.info(
            f"Repository: Location query completed in {
                end_time - start_time:.2f} seconds")
        return result

    @cache.cacheable(lambda self,
                     **kwargs: f"locations_filter:{hash(str(sorted(kwargs.items())))}",
                     ttl=300,
                     )
    def filter_by(self, **kwargs) -> List[Location]:
        ...
        if search_term:
            ...
            query = query.outerjoin(
                LocationDetails,
                Location.location_id == LocationDetails.location_id,
            )
            search_filter = or_(
        ...
        if min_score is not None:
            query = query.join(
                LocationStats,
                Location.location_id == LocationStats.location_id,
            )
        ...
        end_time = time.time()
        logger.info(
            f"Repository: Location filtering completed in {
                end_time -
                start_time:.2f} seconds, found {
                len(result)} locations")
        return result

    @cache.cacheable(
        lambda self, category_id: f"locations_category:{category_id}", ttl=300
    )
    ...
    @cache.cacheable(
        lambda self, region_id: f"locations_region:{region_id}", ttl=300
    )
    ...
    @cache.cacheable(
        lambda self, district_id: f"locations_district:{district_id}", ttl=300
    )
    ...
    @cache.cacheable(
        lambda self, city_id: f"locations_city:{city_id}", ttl=300
    )
    ...
    def assign_inspector(self, location_id: UUID, user_id: UUID) -> None:
        ...
        li = LocationInspector(
            location_id=str(location_id), user_id=str(user_id)
        )
        self.db.merge(li)
    ...
    def get_locations_for_inspector(self, user_id: UUID) -> List[Location]:
        ...
        stmt = (
            ...
            .join(
                LocationInspector,
                Location.location_id == LocationInspector.location_id,
            )

...
class RegionRepository(SQLAlchemyRepository[Region, int]):
    ...
    def get_paginated(self, limit: int, offset: int):
        ...
        total = query.count()
        items = (
            query.order_by(Region.region_id).offset(offset).limit(limit).all()
        )
        return items, total
...
class DistrictRepository(SQLAlchemyRepository[District, int]):
    ...
    def get_paginated(self, limit: int, offset: int):
        ...
        total = query.count()
        items = (
            query.order_by(District.district_id)
            .offset(offset)
            .limit(limit)
            .all()
        )
...
class CityRepository(SQLAlchemyRepository[City, int]):
    ...
    def in_district(self, district_id: int):
        return (
            self.db.query(City).filter(City.district_id == district_id).all()
        )

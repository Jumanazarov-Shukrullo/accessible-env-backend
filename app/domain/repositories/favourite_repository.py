...
class FavouriteRepository(SQLAlchemyRepository[Favourite, int]):
    ...
    def get_user_fav(self, user_id: UUID, location_id: UUID) -> Favourite | None:
        stmt = select(Favourite).where(Favourite.user_id == str(user_id), Favourite.location_id == str(location_id))

...
class UserDomainService:
    ...
    @staticmethod
    def validate_user_registration(username: str, email: str, existing_username: Optional[User], existing_email: Optional[User]) -> None:
    ...
    @staticmethod
    def construct_full_name(first_name: str, surname: str, middle_name: Optional[str] = None) -> str:
    ...
    @staticmethod
    def validate_role_change(actor: User, target: User, new_role_id: int) -> None:
        ...
        if new_role_id == RoleID.SUPERADMIN.value and actor.role_id != RoleID.SUPERADMIN.value:
            raise InvalidRoleAssignment("Only superadmins can promote to superadmin")
    ...
    def validate_invite_permission(inviter: User, target_role_id: int) -> None:
        ...
        if target_role_id not in (RoleID.ADMIN.value, RoleID.SUPERADMIN.value):
            raise InvalidRoleAssignment("Can only invite admins or superadmins")
    ...
    def validate_inspector_assignment(assigner: User, inspector: User) -> None:
        ...
        if inspector.role_id not in [RoleID.SUPERADMIN.value, RoleID.ADMIN.value]:
            raise InvalidRoleAssignment("Only admin and superadmin users can be assigned as inspectors")

...
class TestLocationService:
    ...
    def test_get_location_success(self, location_service, mock_uow, sample_location):
        ...
        mock_uow.locations.get_by_id.assert_called_once_with(sample_location.location_id)
    ...
    def test_update_location_success(self, location_service, mock_uow, sample_location):
        """Test successful location update."""
        update_data = LocationUpdate(
            location_name="Updated Library",
        ...
        result = location_service.update_location(sample_location.location_id, update_data)

        assert result is not None
        mock_uow.locations.update.assert_called_once()
        mock_uow.commit.assert_called_once()

    def test_delete_location_success(self, location_service, mock_uow, sample_location):
        ...
        mock_uow.locations.delete.assert_called_once_with(sample_location.location_id)
        mock_uow.commit.assert_called_once()
    ...
    def test_filter_by_category(self, location_service, mock_uow):
        ...
        mock_uow.locations.filter_by_category.assert_called_once_with(category_id)
    ...
    def test_get_nearby_locations(self, location_service, mock_uow):
        ...
        results = location_service.get_nearby_locations(latitude, longitude, radius_km)

        assert len(results) == 1
        mock_uow.locations.get_nearby.assert_called_once_with(latitude, longitude, radius_km)
    ...
    def test_bulk_update_status(self, location_service, mock_uow):
        ...
        assert result == 3
        mock_uow.locations.bulk_update_status.assert_called_once_with(location_ids, new_status)
        mock_uow.commit.assert_called_once()
    ...
    def test_get_paginated_locations(self, location_service, mock_uow):
        ...
        total_count = 100
        mock_uow.locations.get_paginated.return_value = (mock_locations, total_count)

        results, total = location_service.get_paginated_locations(limit=10, offset=0)

        assert len(results) == 10
        assert total == 100
        mock_uow.locations.get_paginated.assert_called_once_with(limit=10, offset=0)

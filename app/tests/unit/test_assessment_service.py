...
class TestAssessmentService:
    ...
    def test_create_assessment_unauthorized_role(
        ...
        assessment_data = AssessmentCreate(
            location_id=str(uuid4()), assessment_set_id=1
        )
    ...
    def test_get_assessment_success(
        self, assessment_service, mock_uow, sample_assessment
    ):
        ...
        mock_uow.assessments.get_by_id.assert_called_once_with(
            sample_assessment.assessment_id
        )
    ...
    def test_update_assessment_success(
        self, assessment_service, mock_uow, sample_assessment, sample_user
    ):
        """Test successful assessment update."""
        update_data = AssessmentUpdate(
            notes="Updated notes", status="in_progress"
        )
        ...
        result = assessment_service.update_assessment(
            sample_assessment.assessment_id, update_data, sample_user
        )
    ...
    def test_update_assessment_unauthorized_user(
        self, assessment_service, mock_uow, sample_assessment
    ):
        ...
        with pytest.raises(UnauthorizedOperation):
            assessment_service.update_assessment(
                sample_assessment.assessment_id, update_data, unauthorized_user
            )
    ...
    def test_submit_assessment_success(
        ...
        result = assessment_service.submit_assessment(
            sample_assessment.assessment_id, sample_user
        )
    ...
    def test_submit_assessment_wrong_status(
        self, assessment_service, mock_uow, sample_user
    ):
        ...
        with pytest.raises(ValidationError):
            assessment_service.submit_assessment(
                submitted_assessment.assessment_id, sample_user
            )
    ...
    def test_verify_assessment_unauthorized(
        self, assessment_service, mock_uow, sample_assessment, sample_user
    ):
    ...
    def test_delete_assessment_submitted_status(
        self, assessment_service, mock_uow, sample_user
    ):
    ...
    def test_get_assessments_by_location(self, assessment_service, mock_uow):
        ...
        mock_uow.assessments.get_by_location.assert_called_once_with(
            location_id
        )

    def test_get_assessments_by_assessor(
        self, assessment_service, mock_uow, sample_user
    ):
        ...
        results = assessment_service.get_assessments_by_assessor(
            sample_user.user_id
        )

        assert len(results) == 3
        mock_uow.assessments.get_by_assessor.assert_called_once_with(
            sample_user.user_id
        )
    ...
    def test_bulk_update_status(self, assessment_service, mock_uow):
        ...
        new_status = "archived"
        admin_user = User(
            user_id=str(uuid4()), role_id=RoleID.ADMIN.value, is_active=True
        )

        mock_uow.assessments.bulk_update_status.return_value = 3

        result = assessment_service.bulk_update_status(
            assessment_ids, new_status, admin_user
        )

        assert result == 3
        mock_uow.assessments.bulk_update_status.assert_called_once_with(
            assessment_ids, new_status
        )
        mock_uow.commit.assert_called_once()

    def test_bulk_update_status_unauthorized(
        self, assessment_service, mock_uow, sample_user
    ):
        ...
        with pytest.raises(UnauthorizedOperation):
            assessment_service.bulk_update_status(
                assessment_ids, new_status, sample_user
            )

    @patch("app.services.assessment_service.cache")
    def test_caching_integration(
        self, mock_cache, assessment_service, mock_uow, sample_user
    ):

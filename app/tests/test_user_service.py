...
from app.schemas.user_schema import UserCreate, UserUpdate, UserProfileUpdate, InviteCreate
from app.domain.unit_of_work import UnitOfWork
from app.domain.exceptions import UserNotFound, UserAlreadyExists, InvalidCredentials
...
class TestUserService:
    ...
    def test_create_user_with_profile_success(self, user_service, mock_uow, admin_user):
        ...
        # Act
        with patch('app.core.security.security_manager.generate_temp_password') as mock_gen_pass, \
             patch('app.core.security.security_manager.get_password_hash') as mock_hash:
            mock_gen_pass.return_value = "temp123"
            mock_hash.return_value = "hashed_temp123"
            
            user, temp_password = user_service.create_user_with_profile(invite_data, admin_user)
    ...
    def test_create_user_with_existing_email_raises_exception(self, user_service, mock_uow, admin_user):
        ...
        mock_uow.session.query.return_value.filter.return_value.first.return_value = existing_user
    ...
    def test_get_user_with_profile_success(self, user_service, mock_uow, sample_user):
        """Test successful retrieval of user with profile."""
        # Arrange
        mock_uow.session.query.return_value.options.return_value.filter.return_value.first.return_value = sample_user
    ...
    def test_get_user_by_email_success(self, user_service, mock_uow, sample_user):
    ...
    def test_get_user_by_username_success(self, user_service, mock_uow, sample_user):
    ...
    def test_update_user_core_success(self, user_service, mock_uow, sample_user):
        """Test successful core user data update."""
        # Arrange
        user_update = UserUpdate(username="updated_user", email="updated@example.com")
        mock_uow.session.query.return_value.filter.return_value.first.return_value = sample_user
        
        with patch.object(user_service, 'get_user_with_profile') as mock_get:
            mock_get.return_value = Mock()
            
            # Act
            result = user_service.update_user_core(sample_user.user_id, user_update)
    ...
    def test_update_user_profile_success(self, user_service, mock_uow, sample_user):
        """Test successful user profile update."""
        # Arrange
        profile_update = UserProfileUpdate(first_name="Updated", surname="Name")
        mock_profile = Mock(spec=UserProfile)
        mock_uow.session.query.return_value.filter.return_value.first.return_value = mock_profile
        
        with patch.object(user_service, 'get_user_with_profile') as mock_get:
            mock_get.return_value = Mock()
            
            # Act
            result = user_service.update_user_profile(sample_user.user_id, profile_update)
            
            # Assert
            assert result is not None
            mock_uow.commit.assert_called()

    def test_update_user_profile_creates_new_profile(self, user_service, mock_uow, sample_user):
    ...
    def test_authenticate_user_success(self, user_service, mock_uow, sample_user):
        ...
        with patch.object(user_service, 'get_user_by_email') as mock_get, \
             patch('app.core.security.security_manager.verify_password') as mock_verify:
            mock_get.return_value = mock_user_with_security
            mock_verify.return_value = True
            
            # Act
            result = user_service.authenticate_user("test@example.com", "testpass123", "127.0.0.1")
            
            # Assert
            assert result == mock_user_with_security
            mock_uow.commit.assert_called()

    def test_authenticate_user_invalid_password(self, user_service, mock_uow, sample_user):
        ...
        with patch.object(user_service, 'get_user_by_email') as mock_get, \
            ...
            # Act
            result = user_service.authenticate_user("test@example.com", "wrongpass", "127.0.0.1")
    ...
    def test_authenticate_user_account_locked(self, user_service, mock_uow):
        ...
        with patch.object(user_service, 'get_user_by_email') as mock_get:
            ...
            # Act & Assert
            with pytest.raises(HTTPException) as exc_info:
                user_service.authenticate_user("test@example.com", "password", "127.0.0.1")
    ...
    def test_change_password_success(self, user_service, mock_uow, sample_user):
        ...
        mock_uow.session.query.return_value.filter.return_value.first.return_value = mock_user
        
        with patch('app.core.security.security_manager.verify_password') as mock_verify, \
             patch('app.core.security.security_manager.get_password_hash') as mock_hash:
            mock_verify.return_value = True
            mock_hash.return_value = "new_hash"
            
            # Act
            result = user_service.change_password(sample_user.user_id, "oldpass", "newpass")
    ...
    def test_change_password_wrong_current_password(self, user_service, mock_uow, sample_user):
        ...
        with patch('app.core.security.security_manager.verify_password') as mock_verify:
            ...
            # Act & Assert
            with pytest.raises(HTTPException) as exc_info:
                user_service.change_password(sample_user.user_id, "wrongpass", "newpass")
    ...
    def test_deactivate_user_success(self, user_service, mock_uow, admin_user):
        ...
        mock_uow.session.query.return_value.filter.return_value.first.return_value = target_user
    ...
    def test_user_not_found_operations(self, user_service, mock_uow, admin_user):

...
class RoleService:
    ...
    def _get_all_role_permissions_bulk(self) -> Dict[int, List[dict]]:
        ...
        try:
            ...
            # Single query to get all role-permission mappings with permission details
            query_result = (
                self.db.query(
            ...
            end_time = time.time()
            logger.info(f"Bulk loaded permissions for {len(role_permissions)} roles in {(end_time - start_time) * 1000:.2f}ms")
    ...
    def _get_all_user_counts_bulk(self) -> Dict[int, int]:
        ...
        try:
            ...
            end_time = time.time()
            logger.info(f"Bulk loaded user counts in {(end_time - start_time) * 1000:.2f}ms")
    ...
    def list_roles(self) -> List[RoleResponse]:
        ...
        end_time = time.time()
        logger.info(f"Optimized list_roles completed in {(end_time - start_time) * 1000:.2f}ms for {len(role_responses)} roles")
    ...
    def assign_permission_to_role(self, role_id: int, permission_id: int) -> PermissionResponse:
        role = self.repo.get(role_id)
        if not role:
            raise HTTPException(status_code=404, detail="Role not found")
    ...
    def get_statistics(self) -> dict:
        ...
        try:
            ...
            if all_roles:
                sorted_roles = sorted(all_roles, key=lambda r: r.user_count, reverse=True)
            ...
            for role in all_roles:
                percentage = (role.user_count / total_users * 100) if total_users > 0 else 0
            ...
            end_time = time.time()
            logger.info(f"Optimized statistics completed in {(end_time - start_time) * 1000:.2f}ms")

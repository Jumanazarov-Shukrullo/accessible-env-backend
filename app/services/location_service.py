...
from sqlalchemy import and_, or_, func, text, distinct, case, desc, asc, select
...
from app.models.assessment_model import LocationSetAssessment
from app.schemas.location_schema import (
    LocationCreate, LocationUpdate, LocationResponse, LocationDetailResponse,
    LocationListResponse, LocationSearchResponse, LocationFilter, LocationSearch,
    LocationDetailsUpdate, PaginatedLocations
)
...
class LocationService:
    ...
    def _patch_image_urls(self, images):
        ...
        # Or, if the goal was to provide the API-proxied URL, that logic would be different.
        ...
        for img in images:
            ...
            # If the goal is to return the object key itself for the frontend to handle:
            patched_url = img.image_url 

            patched_images.append(
                {
                    "image_id": img.image_id,
                    "image_url": f'{settings.storage.minio_endpoint}/{settings.storage.minio_bucket}/{patched_url}', # Sending object key
    ...
    def create_location(self, location_in: LocationCreate, created_by: User) -> LocationResponse:
        ...
        with self.uow:
            ...
            logger.info(f"Location {location.location_name} created by {created_by.username}")
            return self.get_location_with_details(location.location_id)

    @cache.cacheable(lambda self, location_id: f"location_details:{location_id}", ttl=300)  # 5 minutes cache
    def get_location_with_details(self, location_id: str) -> Optional[LocationResponse]:
        """Get location with all related data"""
        location = self.uow.db.query(Location).options(
            ...
            joinedload(Location.inspectors).joinedload(LocationInspector.user)
        ...
        try:
            ...
            if location.category:
                ...
                while current_cat and current_cat.parent_category_id:
                    parent_cat = self.uow.db.query(Category).filter(Category.category_id == current_cat.parent_category_id).first()
                    if parent_cat:
            ...
            logger.warning(f"Error building breadcrumb for location {location_id}: {e}")
            breadcrumb = []
        ...
        # Create response manually to ensure all nested data is included with proper string conversion
        response_data = {
            ...
            "category_name": location.category.category_name if location.category else None,
            "region_name": location.region.region_name if location.region else None,
        ...
        if location.images:
            for image in location.images:
                ...
                full_image_url = f"http://{settings.storage.minio_endpoint}/{settings.storage.minio_bucket}/{image.image_url}"
            ...
            for inspector in location.inspectors:
                ...
                if inspector.user:
                    ...
                    if inspector.user.profile:
                        inspector_data["first_name"] = inspector.user.profile.first_name
                        inspector_data["surname"] = inspector.user.profile.surname
                        inspector_data["middle_name"] = inspector.user.profile.middle_name
        ...
        if location.images and len(location.images) > 0:
            response_data["primary_image_url"] = f"http://{settings.storage.minio_endpoint}/{settings.storage.minio_bucket}/{location.images[0].image_url}"
        
        return LocationResponse(**response_data)

    @cache.cacheable(lambda self, location_id: f"location:detail:{location_id}", ttl=300)
    def get_location_detail(self, location_id: str) -> Optional[LocationDetailResponse]:
        ...
        with self.uow:
            ...
            # Convert the location object to dict first and ensure UUIDs are strings
            loc_dict = {
                ...
                "details": {
                    "location_id": str(loc.location_id),
                    "contact_info": loc.details.contact_info if loc.details and hasattr(loc.details, 'contact_info') else None,
                    "website_url": loc.details.website_url if loc.details and hasattr(loc.details, 'website_url') else None,
                    "description": loc.details.description if loc.details and hasattr(loc.details, 'description') else None,
                    "operating_hours": loc.details.operating_hours if loc.details and hasattr(loc.details, 'operating_hours') else None,
                    "created_at": loc.details.created_at.isoformat() if loc.details and loc.details.created_at else None,
                    "updated_at": loc.details.updated_at.isoformat() if loc.details and loc.details.updated_at else None,
                } if loc.details else None,
                "stats": {
                    "location_id": str(loc.location_id),
                    "accessibility_score": loc.stats.accessibility_score if loc.stats else None,
                    "total_reviews": loc.stats.total_reviews if loc.stats else 0,
                    "average_rating": loc.stats.average_rating if loc.stats else None,
                    "total_ratings": loc.stats.total_ratings if loc.stats else 0,
                    "last_assessment_date": loc.stats.last_assessment_date.isoformat() if loc.stats and loc.stats.last_assessment_date else None,
                    "updated_at": loc.stats.updated_at.isoformat() if loc.stats and loc.stats.updated_at else None,
                } if loc.stats else None,
                "images": [
                    {
                ...
                "inspectors": [
                    {
                        ...
                        "email": insp.email,
                        "assigned_at": insp.assigned_at.isoformat() if insp.assigned_at else None
            ...
            # Get category path
            if loc.category_id:
                category = self.uow.db.query(Category).filter(Category.category_id == loc.category_id).first()
                if category:
                    ...
                    while current_cat:
                        category_path.insert(0, {
                            "type": "category",
                            "id": current_cat.category_id,
                            "name": current_cat.category_name
                        })
                        if current_cat.parent_category_id:
                            current_cat = self.uow.db.query(Category).filter(
                                Category.category_id == current_cat.parent_category_id
                            ).first()
                        else:
                            current_cat = None
                    breadcrumb.extend(category_path)
            
            # Get region/district/city info for breadcrumb
            if loc.region_id:
                region = self.uow.db.query(Region).filter(Region.region_id == loc.region_id).first()
                if region:
                    breadcrumb.append({
                        "type": "region",
                        "id": region.region_id,
            ...
            if loc.district_id:
                district = self.uow.db.query(District).filter(District.district_id == loc.district_id).first()
                if district:
                    breadcrumb.append({
                        "type": "district", 
            ...
            if loc.city_id:
                city = self.uow.db.query(City).filter(City.city_id == loc.city_id).first()
                if city:
                    breadcrumb.append({
                        "type": "city",
                        "id": city.city_id,
    ...
    def update_location(self, location_id: UUID, location_update: LocationUpdate, updated_by: User) -> LocationResponse:
        """Update location - enhanced to handle both core and details"""
        with self.uow:
            location_id_str = str(location_id)
            
            # Get the location first
            location = self.uow.db.query(Location).filter(Location.location_id == location_id_str).first()
            if not location:
                raise HTTPException(
            ...
            core_fields = ['location_name', 'address', 'latitude', 'longitude', 'category_id', 'region_id', 'district_id', 'city_id', 'status']
            details_fields = ['contact_info', 'website_url', 'operating_hours', 'description']
            ...
            details = self.uow.db.query(LocationDetails).filter(LocationDetails.location_id == location_id_str).first()
            if not details:
            ...
            self.uow.commit()
            logger.info(f"Location {location.location_name} updated by {updated_by.username}")
            return self.get_location_with_details(location_id_str)

    def search_locations(self, search: LocationSearch) -> Tuple[List[LocationSearchResponse], int]:
        ...
        if search.filters:
            if search.filters.category_id:
                query = query.filter(Location.category_id == search.filters.category_id)
            if search.filters.region_id:
                query = query.filter(Location.region_id == search.filters.region_id)
            if search.filters.district_id:
                query = query.filter(Location.district_id == search.filters.district_id)
            if search.filters.city_id:
                query = query.filter(Location.city_id == search.filters.city_id)
            ...
            if search.filters.min_accessibility_score:
                query = query.join(LocationStats).filter(LocationStats.accessibility_score >= search.filters.min_accessibility_score)
            if search.filters.max_accessibility_score:
                query = query.join(LocationStats).filter(LocationStats.accessibility_score <= search.filters.max_accessibility_score)
        ...
        if search.center_lat and search.center_lng and search.radius_km:
            ...
            distance_expr = func.acos(
                ...
                func.cos(func.radians(Location.longitude) - func.radians(search.center_lng)) +
        ...
        for result in locations:
            ...
            if location.images and len(location.images) > 0:
                ...
                primary_image = f"http://{settings.storage.minio_endpoint}/{settings.storage.minio_bucket}/{image_key}"
            ...
            if location.category:
                ...
                from app.models.category_model import Category
                category = self.uow.db.query(Category).filter(Category.category_id == location.category_id).first()
            ...
            response_data = {
                ...
                "accessibility_score": location.stats.accessibility_score if location.stats else None,
                "average_rating": location.stats.average_rating if location.stats else None,
    ...
    def get_locations_paginated(self, page: int = 1, size: int = 50, filters: LocationFilter = None) -> PaginatedLocations:
        ...
        if filters:
            if filters.category_id:
                query = query.filter(Location.category_id == filters.category_id)
            if filters.region_id:
                query = query.filter(Location.region_id == filters.region_id)
            if filters.district_id:
                query = query.filter(Location.district_id == filters.district_id)
            if filters.city_id:
            ...
            if filters.min_accessibility_score:
                query = query.join(LocationStats).filter(LocationStats.accessibility_score >= filters.min_accessibility_score)
            if filters.has_assessments is not None:
                if filters.has_assessments:
                    query = query.join(LocationStats).filter(LocationStats.last_assessment_date.isnot(None))
                else:
                    query = query.join(LocationStats).filter(LocationStats.last_assessment_date.is_(None))
        ...
        category_ids = {loc.category_id for loc in locations if loc.category_id}
        ...
        if category_ids:
            from app.models.category_model import Category
            categories = self.uow.db.query(Category).filter(Category.category_id.in_(category_ids)).all()
            categories_map = {cat.category_id: cat.category_name for cat in categories}
        
        if region_ids:
            from app.models.region_model import Region
            regions = self.uow.db.query(Region).filter(Region.region_id.in_(region_ids)).all()
        ...
        for location in locations:
            ...
            if location.category:
                category_name = location.category.category_name
            elif location.category_id and location.category_id in categories_map:
            ...
            if location.region:
                ...
                # Last resort fallback
                from app.models.region_model import Region
                region = self.uow.db.query(Region).filter(Region.region_id == location.region_id).first()
            ...
            response_data = {
                ...
                "accessibility_score": location.stats.accessibility_score if location.stats else None,
                "average_rating": location.stats.average_rating if location.stats else None,
                "total_reviews": location.stats.total_reviews if location.stats else 0,
                "category_name": category_name,
    ...
    def add_location_to_favourites(self, location_id: str, user_id: str) -> bool:
        """Add location to user's favourites"""
        with self.uow:
            # Check if already exists
            existing = self.uow.db.query(Favourite).filter(
                and_(Favourite.location_id == location_id, Favourite.user_id == user_id)
            ).first()
    ...
    def remove_location_from_favourites(self, location_id: str, user_id: str) -> bool:
        """Remove location from user's favourites"""
        with self.uow:
            favourite = self.uow.db.query(Favourite).filter(
                and_(Favourite.location_id == location_id, Favourite.user_id == user_id)
            ).first()
    ...
    def rate_location(self, location_id: str, user_id: str, rating_value: int) -> bool:
        """Rate a location and update stats"""
        with self.uow:
            # Check if user already rated this location
            existing_rating = self.uow.db.query(LocationRating).filter(
                and_(LocationRating.location_id == location_id, LocationRating.user_id == user_id)
            ).first()
            ...
            self._update_location_rating_stats(location_id, rating_value, old_value)
            
            self.uow.commit()
            return True

    def _update_location_rating_stats(self, location_id: str, new_rating: int, old_rating: int = None):
        """Update location rating statistics"""
        stats = self.uow.db.query(LocationStats).filter(LocationStats.location_id == location_id).first()
        if not stats:
            return

        # Get all ratings for this location
        ratings = self.uow.db.query(LocationRating.rating_value).filter(
    ...
    def bulk_update_location_status(self, location_ids: List[str], status: str, updated_by: User) -> int:
        """Bulk update location status"""
        with self.uow:
            updated_count = self.uow.db.query(Location).filter(
                Location.location_id.in_(location_ids)
            ).update({"status": status}, synchronize_session=False)
            
            self.uow.commit()
            logger.info(f"Bulk updated {updated_count} locations to status {status} by {updated_by.username}")
            return updated_count

    def delete_location(self, location_id: str, deleted_by: User) -> bool:
        """Delete location and all related data (cascading)"""
        with self.uow:
            location = self.uow.db.query(Location).filter(Location.location_id == location_id).first()
            if not location:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail="Location not found"
            ...
            location_name = location.location_name
            self.uow.db.delete(location)  # Cascade will handle details, stats, images, etc.
            self.uow.commit()
            
            logger.info(f"Location {location_name} deleted by {deleted_by.username}")
            return True

    def get_user_favourites(self, user_id: str, page: int = 1, size: int = 50) -> PaginatedLocations:
        """Get user's favourite locations"""
        query = self.uow.db.query(Location).join(Favourite).filter(
    ...
    def assign_inspector(self, current_user: User, loc_id: UUID, inspector_id: UUID):
        """Assign an inspector to a location - only superadmin can do this."""
        # Only superadmin (role_id=1) can assign inspectors
        if current_user.role_id != 1:
            raise HTTPException(status_code=403, detail="Only superadmin can assign inspectors")
        ...
        # Only users with role_id=1 (superadmin) or role_id=2 (admin) can be assigned as inspectors
        if inspector_user.role_id not in [1, 2]:
            raise HTTPException(status_code=400, detail="Only admin and superadmin users can be assigned as inspectors")
        
        # Admin users can't assign themselves as inspectors
        if current_user.user_id == inspector_id and current_user.role_id == 2:
            raise HTTPException(status_code=400, detail="Admin users cannot assign themselves as inspectors")
        
        with self.uow:
            self.uow.locations.assign_inspector(self.ensure_uuid(loc_id), self.ensure_uuid(inspector_id))
            self.uow.commit()

    def unassign_inspector(self, current_user: User, loc_id: UUID, inspector_id: UUID):
        """Remove an inspector from a location - only superadmin can do this."""
        # Only superadmin (role_id=1) can remove inspectors
        if current_user.role_id != 1:
            raise HTTPException(status_code=403, detail="Only superadmin can remove inspectors")
        
        with self.uow:
            self.uow.locations.unassign_inspector(self.ensure_uuid(loc_id), self.ensure_uuid(inspector_id))
            self.uow.commit()
    ...
    def update_accessibility_score(self, location_id: UUID) -> Optional[float]:
        ...
        with self.uow:
            ...
            result = self.uow.db.execute(text("""
            ...
            stats = self.uow.db.query(LocationStats).filter(LocationStats.location_id == str(location_id)).first()
            if stats:
                stats.accessibility_score = float(avg_score)
                self.uow.commit()
                logger.info(f"Updated accessibility_score for location {location_id}: {avg_score}")
                ...
                self.uow.db.add(stats)
                self.uow.commit()
                logger.info(f"Created stats record and set accessibility_score for location {location_id}: {avg_score}")
                return float(avg_score)

    def get_popular_locations(self, limit: int = 10) -> List[LocationResponse]:
        """Get popular locations with complete data."""
        locations = self.uow.db.query(Location).options(
            joinedload(Location.details),
            joinedload(Location.stats),
            joinedload(Location.images),
            joinedload(Location.category),
        ...
        # Convert to LocationResponse objects with proper UUID to string conversion
        response_locations = []
        for location in locations:
    ...
    def get_recently_rated_locations(self, limit: int = 10) -> List[LocationResponse]:
    ...
    def add_rating(self, location_id: UUID, rating: float, comment: str = None, current_user: User = None):
        ...
        rating_service = RatingService(self.uow)
        payload = RatingCreate(location_id=location_id, rating=rating, comment=comment)
    ...
    def get_location_stats(self, location_id: UUID) -> dict:
        ...
        # Get favorites count (approximate - you might want to add this method to social service)
        ...
        accessibility_score = location.stats.accessibility_score if location and location.stats else 0.0
    ...
    def get_favorite_status(self, location_id: UUID, current_user: User) -> dict:
        """Get favorite status for a location for the current user."""
        existing_fav = self.uow.favourites.get_user_fav(current_user.user_id, location_id)
    ...
    def update_location_average_rating(self, location_id: UUID):
        ...
        rating_service = RatingService(self.uow)
        stats = rating_service.get_location_rating_stats(location_id)
    ...
    def add_to_favorites(self, location_id: UUID, current_user: User):
        ...
        if existing_fav:
            return {"message": "Location already in favorites", "is_favorited": True}
    ...
    def remove_from_favorites(self, location_id: UUID, current_user: User):
        ...
        if not existing_fav:
            return {"message": "Location not in favorites", "is_favorited": False}
    ...
    def _process_locations_for_output(self, locations: List[Location | dict]) -> List[dict]:
        processed_locations = []
        for loc_entry in locations:
            ...
            if isinstance(loc_entry, Location):
                ...
                loc_data = {
                    ...
                    "accessibility_score": getattr(loc_entry.stats, 'accessibility_score', None) if hasattr(loc_entry, 'stats') and loc_entry.stats else None,
                    "average_rating": getattr(loc_entry.stats, 'average_rating', None) if hasattr(loc_entry, 'stats') and loc_entry.stats else None,
                    "total_reviews": getattr(loc_entry.stats, 'total_reviews', 0) if hasattr(loc_entry, 'stats') and loc_entry.stats else 0,
                    "category_name": getattr(loc_entry.category, 'category_name', None) if hasattr(loc_entry, 'category') and loc_entry.category else None,
                    "region_name": getattr(loc_entry.region, 'region_name', None) if hasattr(loc_entry, 'region') and loc_entry.region else None,
                    "primary_image_url": loc_entry.images[0].image_url if hasattr(loc_entry, 'images') and loc_entry.images else None
                }
            else:
            ...
            if loc_data.get("primary_image_url"):
                loc_data["primary_image_url"] = f'{settings.storage.minio_endpoint}/{settings.storage.minio_bucket}/{loc_data["primary_image_url"]}'
    ...
    @cache.cacheable(lambda self, category_id: f"locations:category:{category_id}", ttl=300)
    def get_locations_by_category(self, category_id: int) -> List[LocationResponse]:
        ...
        locations = self.uow.db.query(Location).options(
            ...
            joinedload(Location.district),
            joinedload(Location.city)
        ).filter(Location.category_id == category_id).all()
        
        return [LocationResponse.from_orm(location) for location in locations]

    @cache.cacheable(lambda self, region_id: f"locations:region:{region_id}", ttl=300)
    def get_locations_by_region(self, region_id: int) -> List[LocationResponse]:
        ...
        locations = self.uow.db.query(Location).options(
            ...
            joinedload(Location.district),
            joinedload(Location.city)
        ).filter(Location.region_id == region_id).all()
        
        return [LocationResponse.from_orm(location) for location in locations]

    @cache.cacheable(lambda self, district_id: f"locations:district:{district_id}", ttl=300)
    def get_locations_by_district(self, district_id: int) -> List[LocationResponse]:
        ...
        locations = self.uow.db.query(Location).options(
            ...
            joinedload(Location.district),
            joinedload(Location.city)
        ).filter(Location.district_id == district_id).all()
        
        return [LocationResponse.from_orm(location) for location in locations]

    @cache.cacheable(lambda self, city_id: f"locations:city:{city_id}", ttl=300)
    def get_locations_by_city(self, city_id: int) -> List[LocationResponse]:
        ...
        locations = self.uow.db.query(Location).options(
            ...
            joinedload(Location.district),
            joinedload(Location.city)
        ).filter(Location.city_id == city_id).all()

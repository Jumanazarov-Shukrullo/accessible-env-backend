...
class ReviewService:
    ...
    # ---------------- Reviews ----------------------------------------
    def create_review(self, payload: ReviewSchema.Create, user: User) -> Review:
        with self.uow:
    ...
    def update_review(self, review_id: int, payload: ReviewSchema.Update, user: User) -> Review:
    ...
    # ---------------- Ratings ----------------------------------------
    def rate_location(self, payload: RatingCreate, user: User) -> LocationRating:
        if not 1.0 <= payload.rating <= 5.0:
            raise HTTPException(400, "Rating must be 1.0-5.0")
        
        with self.uow:
            # Check if user already rated this location
            existing = self.uow.ratings.get_user_rating_for_location(user.user_id, payload.location_id)
            if existing:

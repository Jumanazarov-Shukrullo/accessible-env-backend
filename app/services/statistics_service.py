...
from app.schemas.statistics_schema import CategoryStats, LocationStats, RegionStats, BuildingAssessmentSummary
...
class DashboardStatisticsService:
    ...
    def get_by_category_and_region(self) -> List[CategoryStats]:
        ...
        with self.uow:
            ...
            # First pass: collect region and category names, and aggregate stats
            for summary in summaries:
                ...
                if not category_data[cat_id]['category_name']:
                    category_data[cat_id]['category_name'] = summary['category_name']
                if not category_data[cat_id]['regions'][region_id]['region_name']:
                    category_data[cat_id]['regions'][region_id]['region_name'] = summary['region_name']
                ...
                if status == 'new':
                    ...
                    # Treat active status as old buildings (existing/operational)
                    stats['old_buildings']['total'] += 1
                    if is_good:
                        stats['old_buildings']['good'] += 1
                    else:
            ...
            for cat_id, cat_data in category_data.items():
                ...
                for region_id, region_data in cat_data['regions'].items():
                    ...
                    regions_list.append(RegionStats(
                        region_id=region_id,
    ...
    def get_building_assessment_summaries(self) -> List[BuildingAssessmentSummary]:
        """Get building assessment summaries for classification."""
        with self.uow:
            summaries = self._get_building_summaries_data()
            return [BuildingAssessmentSummary(**summary) for summary in summaries]

    def get_category_region_details(self, category_id: int, region_id: int) -> LocationStats:
        """Get detailed statistics for a specific category and region."""
        with self.uow:
            summaries = self._get_building_summaries_data()
            
            # Filter by category and region
            filtered = [s for s in summaries if s['category_id'] == category_id and s['region_id'] == region_id]

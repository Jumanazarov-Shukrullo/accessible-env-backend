...
from app.schemas.rating_schema import RatingCreate, RatingUpdate, RatingOut, LocationRatingStats
...
class RatingService:
    ...
    def add_rating(self, data: RatingCreate, current_user: User) -> RatingOut:
        ...
        with self.uow:
            ...
            if existing_rating:
                ...
                self.uow.commit()
                logger.info(f"Updated rating for location {data.location_id} by user {current_user.user_id}")
                ...
                self.uow.commit()
                logger.info(f"Added new rating for location {data.location_id} by user {current_user.user_id}")
                return self._format_rating_output(new_rating)

    def get_user_rating(self, location_id: UUID, current_user: User) -> Optional[RatingOut]:
        """Get the current user's rating for a location."""
        rating = self.uow.ratings.get_user_rating_for_location(current_user.user_id, location_id)
        if not rating:
            return None
        return self._format_rating_output(rating)

    def get_location_ratings(self, location_id: UUID, limit: int = 50) -> List[RatingOut]:
    ...
    def get_location_rating_stats(self, location_id: UUID) -> LocationRatingStats:
        """Get rating statistics for a location."""
        average_rating = self.uow.ratings.get_average_rating_for_location(location_id)
        total_ratings = self.uow.ratings.get_rating_count_for_location(location_id)
        
        # Get rating distribution
        ratings = self.uow.ratings.get_ratings_for_location(location_id, limit=1000)
    ...
    def delete_rating(self, location_id: UUID, current_user: User) -> bool:
        ...
        with self.uow:
            ...
            self.uow.commit()
            logger.info(f"Deleted rating for location {location_id} by user {current_user.user_id}")
            return True
    ...
    def _format_rating_output(self, rating: LocationRating) -> RatingOut:
        ...
        if hasattr(rating, 'user') and rating.user:
            ...
            if rating.user.profile:
                ...
                middle = f" {profile.middle_name}" if profile.middle_name else ""
                user_full_name = f"{profile.first_name}{middle} {profile.surname}".strip()

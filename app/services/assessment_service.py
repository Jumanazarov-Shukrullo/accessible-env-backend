...
from app.schemas.assessment_set_schema import AssessmentSetSchema, AssessmentSetCreate
...
class AssessmentService:
    ...
    def create(self, payload: AssessmentSchema.Create, assessor: User) -> LocationSetAssessment:
        with self.uow:
    ...
    def submit(self, assessment_id: int, user: User):
        with self.uow:
            ...
            if header.assessor_id != str(user.user_id) and user.role_id not in (1, 2):
                raise HTTPException(status_code=403, detail="Not owner")
            # Allow both draft and pending statuses to be submitted
            if header.status != AssessmentStatus.draft and header.status != "pending":
                raise HTTPException(400, f"Only draft/pending can be submitted, current status: {header.status}")
    ...
    def reject(self, assessment_id: int, verifier: User, reason: str = None):
        ...
        logger.info(f"Rejecting assessment {assessment_id} by {verifier.user_id}")
    ...
    def reassess(self, assessment_id: int, user: User):
        ...
        logger.info(f"Reassessing assessment {assessment_id} by {user.user_id}")

        with self.uow:
            assessment = self._get_or_404(assessment_id)
            
            # Check if user is authorized (owner or admin)
            if assessment.assessor_id != str(user.user_id) and user.role_id not in (1, 2):
                raise HTTPException(status_code=403, detail="Not authorized to reassess this assessment")
            
            # Check if assessment is in rejected status
            if assessment.status != AssessmentStatus.rejected:
                raise HTTPException(status_code=400, detail="Only rejected assessments can be reassessed")
            ...
            self.uow.commit()
            logger.info(f"Assessment {assessment_id} successfully reopened for editing")
    ...
    def delete(self, assessment_id: int, user: User):
        ...
        with self.uow:
            ...
            if assessment.status == "verified" and user.role_id not in [1, 2]:  # Only superadmin/admin can delete verified
                raise HTTPException(status_code=403, detail="Cannot delete verified assessments")
            ...
            for detail in details:
                ...
                images_query = select(AssessmentImage).where(
                    AssessmentImage.assessment_detail_id == detail.location_assessment_id
                )
    ...
    def get_assessment(self, assessment_id: int) -> Optional[LocationSetAssessment]:
        ...
        # No need to query separate verification table since fields are embedded
        return assessment

    def list_assessments(self):
        """List all assessments with proper string formatting for IDs"""
        with self.uow:
            # Use query with joins to get location and assessment set information
            query = (
                select(LocationSetAssessment)
                .options(
                    joinedload(LocationSetAssessment.location),
            ...
            for assessment in assessments_db:
                ...
                assessment_dict = {
                    ...
                    "verifier_id": str(assessment.verifier_id) if assessment.verifier_id else None,
                    # Add location information
                    "location_name": assessment.location.location_name if assessment.location else "Unknown Location",
                    "location_address": assessment.location.address if assessment.location else None,
                    # Add assessment set information
                    "assessment_set_name": assessment.assessment_set.set_name if assessment.assessment_set else "Standard Assessment",
    ...
    def get_assessment_details(self, assessment_id: int) -> List[LocationAssessment]:
        ...
        try:
            ...
            result = self.uow.db.execute(query, {"assessment_id": assessment_id})
            ...
            for row in rows:
                detail = LocationAssessment(
                    ...
                    is_corrected=row.is_corrected if row.is_corrected is not None else False,
                ...
                try:
                    ...
                    """)
                    column_check = self.uow.db.execute(check_column_query).fetchone()
                    ...
                    images_result = self.uow.db.execute(images_query, {"detail_id": detail.assessment_detail_id})
                    ...
                    logger.warning(f"Error loading images for detail {detail.assessment_detail_id}: {img_error}")
            ...
            for detail in details:
                try:
                    ...
                    if comment_row:
                        ...
                        comment_text = comment_row.comment_text.replace(criterion_prefix, "")
                        setattr(detail, "admin_comments", comment_text)
                    
                except Exception as comment_error:
                    logger.warning(f"Error loading comments for criterion {detail.criterion_id}: {comment_error}")
                    
            logger.info(f"Successfully loaded {len(details)} details for assessment {assessment_id}")
            return details
            
        except Exception as e:
            logger.error(f"Error in get_assessment_details for assessment {assessment_id}: {e}")
            ...
            try:
                ...
                logger.warning(f"Used fallback query for assessment {assessment_id}, loaded {len(details)} details")
                return details
                
            except Exception as fallback_error:
                logger.error(f"Fallback query also failed for assessment {assessment_id}: {fallback_error}")
                raise HTTPException(status_code=500, detail="Failed to load assessment details")
    ...
    # Criteria Management
    def create_criterion(self, data: AccessibilityCriteriaCreate) -> AccessibilityCriteria:
    ...
    def update_criterion(self, criterion_id: int, data) -> Optional[AccessibilityCriteria]:
    ...
    def delete_criterion(self, criterion_id: int) -> bool:
        ...
        with self.uow:
            ...
            query = select(SetCriteria).where(SetCriteria.criterion_id == criterion_id)
            result = self.uow.db.execute(query)
            if result.first():
                raise HTTPException(
                    status_code=400, 
    ...
    @cache.cacheable(lambda self: "criteria:list", ttl=3600)  # Cache for 1 hour
    def list_criteria(self) -> List[AccessibilityCriteria]:
        """List all accessibility criteria"""
        query = select(AccessibilityCriteria).order_by(AccessibilityCriteria.criterion_id)
        result = self.uow.db.execute(query)
        return list(result.scalars().all())

    @cache.cacheable(lambda self, criterion_id: f"criteria:{criterion_id}", ttl=3600)  # Cache for 1 hour
    def get_criterion(self, criterion_id: int) -> Optional[AccessibilityCriteria]:
        """Get details of a specific criterion"""
        query = select(AccessibilityCriteria).where(AccessibilityCriteria.criterion_id == criterion_id)
        result = self.uow.db.execute(query)
        return result.scalar_one_or_none()

    # Assessment Sets Management
    def create_assessment_set(self, data: AssessmentSetCreate) -> AssessmentSet:
        """Create a new assessment set"""
        with self.uow:
            assessment_set = AssessmentSet(
                set_name=data.set_name, description=data.description, version=data.version, is_active=True
            )
    ...
    def update_assessment_set(self, set_id: int, data: AssessmentSetSchema.Update) -> AssessmentSet:
    ...
    @cache.cacheable(lambda self: "assessment_sets:list", ttl=3600)  # Cache for 1 hour
    ...
    @cache.cacheable(lambda self, set_id: f"assessment_sets:{set_id}", ttl=3600)  # Cache for 1 hour
    def get_assessment_set(self, set_id: int) -> Optional[AssessmentSet]:
        ...
        query = (
            ...
            .options(joinedload(AssessmentSet.set_criteria).joinedload(SetCriteria.criteria))
        ...
        if assessment_set:
            ...
            # Process criteria to include required fields from the criterion relationship
    ...
    @cache.cacheable(lambda self, set_id: f"assessment_sets:{set_id}:criteria", ttl=3600)  # Cache for 1 hour
    def get_set_criteria(self, set_id: int):
        with self.uow:
            ...
            query = (
                ...
                .join(AccessibilityCriteria, SetCriteria.criterion_id == AccessibilityCriteria.criterion_id)
    ...
    def add_criterion_to_set(self, set_id: int, data: SetCriteriaCreate) -> SetCriteria:
        ...
        with self.uow:
            ...
            existing = (
                ...
                .filter(SetCriteria.set_id == set_id, SetCriteria.criterion_id == data.criterion_id)
                .first()
            )
            ...
            set_criterion = SetCriteria(set_id=set_id, criterion_id=data.criterion_id, sequence=data.sequence)
    ...
    def create_location_assessment_direct(
        ...
        with self.uow:
            ...
            # c) Seed detail rows - only for selected criteria or all if none specified
            if criterion_ids:
                ...
                if invalid_criteria:
                    raise HTTPException(
                        400, 
                        f"Criteria {list(invalid_criteria)} are not part of assessment set {set_id}"
                    )
                
                # Create details only for selected criteria
                selected_criteria = [sc for sc in set_criteria if sc.criterion_id in criterion_ids]
            else:
                # Create details for all criteria in the set (original behavior)
                selected_criteria = (
                    self.uow.db.query(SetCriteria)
                    .filter(SetCriteria.set_id == set_id)
                    .all()
    ...
    def list_location_assessments(self, location_id: UUID) -> List[LocationSetAssessment]:
        ...
        query = (
            ...
            .options(
                ...
                joinedload(LocationSetAssessment.assessor).joinedload(User.profile),
                # Load verifier with profile
                joinedload(LocationSetAssessment.verifier).joinedload(User.profile)
            )
    ...
    def add_assessment_detail(self, assessment_id: int, data: LocationAssessmentCreate) -> LocationAssessment:
        ...
        with self.uow:
            ...
            set_criterion = (
                self.uow.db.query(SetCriteria)
                .filter(SetCriteria.set_id == assessment.set_id, SetCriteria.criterion_id == data.criterion_id)
                .first()
            )
            ...
            if not set_criterion:
                raise HTTPException(
                    status_code=400,
                    detail=f"Criterion {data.criterion_id} is not part of assessment set {assessment.set_id}",
                )
    ...
    def submit_assessment(self, assessment_id: int) -> LocationSetAssessment:
        ...
        with self.uow:
            ...
            # Get the actual details that exist for this assessment (not all criteria in the set)
            existing_details = self.uow.db.query(LocationAssessment).filter(
                LocationAssessment.location_set_assessment_id == assessment_id
            ).all()
            ...
            if unscored_details:
                ...
                for detail in unscored_details:
                    criterion = self.uow.db.get(AccessibilityCriteria, detail.criterion_id)
                    criterion_names.append(criterion.criterion_name if criterion else f"Criterion {detail.criterion_id}")
                
                raise HTTPException(
                    status_code=400,
                    detail=f"Assessment is incomplete. Please score: {', '.join(criterion_names)}"
                )
            ...
            for detail in existing_details:
                image_count = (
                    ...
                    .filter(AssessmentImage.assessment_detail_id == detail.assessment_detail_id)
                    .scalar()
                )
                if image_count == 0:
                    criterion = self.uow.db.get(AccessibilityCriteria, detail.criterion_id)
                    criterion_name = criterion.criterion_name if criterion else f"Criterion {detail.criterion_id}"
            ...
            assessment.overall_score = (total_score / total_possible) * 10  # Scale to 0-10
    ...
    def verify_assessment(
        self, assessment_id: int, verifier_id: UUID, data: AssessmentVerificationCreate
    ) -> LocationSetAssessment:
        ...
        with self.uow:
            ...
            if assessment.status != "submitted":
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot verify assessment with status '{assessment.status}'. Assessment must be submitted.",
                )
            ...
            if data.is_verified:
                ...
                location_service.update_accessibility_score(UUID(assessment.location_id))
    ...
    def get_available_locations(self) -> List[dict]:
        ...
        with self.uow:
            ...
            locations = self.uow.db.query(
                self.uow.locations.model.location_id, self.uow.locations.model.location_name
            ).all()

            # Format for dropdown use
            return [{"id": str(loc.location_id), "name": loc.location_name} for loc in locations]

    def fix_assessment_score(self, assessment_id: int) -> LocationSetAssessment:
        ...
        with self.uow:
            ...
            total_score = (
                ...
                .filter(LocationAssessment.location_set_assessment_id == assessment_id)
                .scalar()
                or 0
            )
            ...
            total_possible = (
                ...
                .join(LocationAssessment, LocationAssessment.criterion_id == AccessibilityCriteria.criterion_id)
                .filter(LocationAssessment.location_set_assessment_id == assessment_id)
                .scalar()
            ...
            if assessment.status in ["submitted", "verified"] and not assessment.submitted_at:
                assessment.submitted_at = dt.utcnow()
            
            self.uow.commit()
            
            # Update location's accessibility_score if this is a verified assessment

...
class Cache:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            try:
                ...
                logger.info(
                    f"Redis cache initialized with URL: {
                        settings.cache.redis_url}")
            except redis.RedisError as e:
                logger.warning(
                    f"Failed to initialize Redis cache: {
                        str(e)}. Caching will be disabled.")
                cls._instance._client = None
        return cls._instance

    def _serialize_value(self, value: Any) -> Any:
        """Recursively convert SQLAlchemy objects and Pydantic models to dictionaries and prepare for JSON serialization."""
        # Handle None values
        if value is None:
            return None
        ...
        if hasattr(value, "__table__"):
            ...
            for column in value.__table__.columns:
                obj_dict[column.name] = self._serialize_value(
    ...
    def get(self, key: str):
        ...
        try:
            ...
            logger.warning(f"Redis connection error during cache retrieval: {str(e)}")
            return None
    ...
    def set(self, key: str, value, ttl: int | None = None):
        ...
        try:
            ...
            self._client.set(
                key,
                json.dumps(serialized_value, cls=UUIDEncoder),
                ex=ttl or self._ttl,
            )
            logger.debug(f"Set cache for key: {key}, TTL: {ttl or self._ttl}s")
        except redis.ConnectionError as e:
            logger.warning(f"Redis connection error during cache set: {str(e)}")
        except Exception as e:
            logger.error(f"Error setting cache: {str(e)}")

    def invalidate(self, key_prefix: str):
        if self._client is None:
            logger.debug(f"Cache not available, skipping invalidation for: {key_prefix}")
            return
        ...
        try:
            ...
            if keys:
                ...
                logger.debug(
                    f"Invalidated {len(keys)} keys with prefix: {key_prefix}"
                )
        except redis.ConnectionError as e:
            logger.warning(f"Redis connection error during cache invalidation: {str(e)}")
        except Exception as e:

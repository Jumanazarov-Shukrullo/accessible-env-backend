...
class UoWDependency:
    ...
    def __call__(self, db: Session = Depends(get_db)) -> Generator[UnitOfWork, None, None]:
...
def require_roles(allowed_roles: List[int]):
    ...
    async def _dependency(current_user: User = Depends(auth_manager.get_current_user)) -> User:

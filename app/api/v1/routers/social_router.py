...
class SocialRouter:
    def __init__(self):
        self.router = APIRouter(prefix="/social", tags=["Comments & Favourites"])
        self._register()

    def _register(self):
        # Comments
        self.router.post("/comments", response_model=CommentSchema.Out, status_code=201)(self._add_comment)
        self.router.get("/comments", response_model=list[CommentSchema.Out])(self._get_thread)

        # Favourites
        self.router.post("/favourites/toggle", status_code=200)(self._toggle_favourite)
        self.router.get("/favourites", response_model=list[FavouriteSchema.Out])(self._list_favs)
    ...
    async def _list_favs(
        self, uow: UnitOfWork = Depends(get_uow), current: User = Depends(auth_manager.get_current_user)
    ):

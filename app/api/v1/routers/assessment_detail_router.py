...
class AssessmentDetailRouter:
    def __init__(self):
        self.router = APIRouter(
            prefix="/assessment-details", tags=["Assessment Details"]
        )
        self._register()

    def _register(self):
        self.router.post("/images/presign", status_code=200)(
            self._presign_image
        )
        self.router.post("/images/metadata", response_model=ImageSchema.Out)(
            self._add_image_metadata
        )
        self.router.post(
            "/", response_model=DetailSchema.Out, status_code=201
        )(self._add_detail)
        self.router.put("/{detail_id}", response_model=DetailSchema.Out)(
            self._update_detail
        )

        self.router.post("/verify", response_model=VerificationSchema.Out)(
            self._verify
        )
        self.router.post(
        ...
        self.router.post(
            "/{detail_id}/admin-comment", response_model=CommentSchema.Out
        )(self._add_admin_comment)
        self.router.get(
            "/{detail_id}/admin-comments",
            response_model=List[CommentSchema.Out],
        )(self._get_admin_comments)
        self.router.get(
            "/{detail_id}/images", response_model=List[ImageSchema.Out]
        )(self._get_images)

        # Image deletion endpoint
        self.router.delete("/{detail_id}/images/{image_id}", status_code=204)(
            self._delete_image
        )

        # Direct image upload endpoint
        self.router.post("/{detail_id}/images", status_code=201)(
            self._upload_image_direct
        )

        # Mark as corrected
        self.router.post(
            "/{detail_id}/mark_correct", response_model=DetailSchema.Out
        )(self._mark_correct)
    ...
    async def _update_detail(
        ...
        return AssessmentDetailService(uow).update_detail(
            detail_id, payload, current
        )
    ...
    async def _add_admin_comment(
        ...
        # Create a comment for the parent assessment using the
        # AssessmentComment table
        comment_data = {
        ...
        return {
            ...
            # Placeholder - actual value doesn't matter for UI
    ...
    async def _get_admin_comments(
        ...
        query = (
            ...
            .where(
                AssessmentComment.location_set_assessment_id
        ...
        criterion_prefix = f"[Criterion {detail.criterion_id}] "
        filtered_comments = [
            c for c in comments if criterion_prefix in c.comment_text
        ]
    ...
    async def _delete_image(
        ...
        return AssessmentDetailService(uow).delete_image(
            detail_id, image_id, current
        )
    ...
    async def _upload_image_direct(
        ...
        return AssessmentDetailService(uow).upload_image_direct(
            detail_id, file, current
        )

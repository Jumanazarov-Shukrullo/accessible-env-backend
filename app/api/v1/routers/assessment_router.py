...
from app.models.assessment_model import LocationAssessment, AccessibilityCriteria
...
class AssessmentRouter:
    ...
    def _register(self):
        self.router.post("/", response_model=AssessmentSchema.Out, status_code=status.HTTP_201_CREATED)(self._create)
        self.router.get("/", response_model=list[AssessmentSchema.Out])(self._list)
        self.router.post("/{assessment_id}/submit", status_code=204)(self._submit)
        self.router.post("/{assessment_id}/verify", status_code=204)(self._verify)
        self.router.post("/{assessment_id}/reject", status_code=204)(self._reject)
        self.router.post("/{assessment_id}/reassess", status_code=204)(self._reassess)
        self.router.delete("/{assessment_id}", status_code=204)(self._delete)
        self.router.post("/criteria", response_model=AccessibilityCriteriaResponse)(self._create_criterion)
        self.router.get("/criteria", response_model=List[AccessibilityCriteriaResponse])(self._list_criteria)
        self.router.get("/criteria/{criterion_id}", response_model=AccessibilityCriteriaResponse)(self._get_criterion)
        self.router.post("/sets", response_model=AssessmentSetResponse)(self._create_assessment_set)
        self.router.get("/sets", response_model=List[AssessmentSetResponse])(self._list_assessment_sets)
        self.router.get("/sets/{set_id}", response_model=AssessmentSetResponse)(self._get_assessment_set)
        self.router.get("/sets/{set_id}/criteria", response_model=List[dict])(self._get_set_criteria)
        self.router.post("/sets/{set_id}/criteria", status_code=status.HTTP_201_CREATED)(self._add_criterion_to_set)

        # Add direct create endpoint that doesn't need location in URL
        self.router.post("/create", response_model=LocationSetAssessmentResponse, status_code=status.HTTP_201_CREATED)(
            self._create_location_assessment_direct
        )

        # Add endpoint to get locations for dropdown
        self.router.get("/available-locations", response_model=List[dict])(self._get_available_locations)

        # Connect location assessment routes to their handlers
        self.router.post("/locations/{location_id}/assessments", response_model=LocationSetAssessmentResponse)(
            self._create_location_assessment
        )
        self.router.get("/locations/{location_id}/assessments", response_model=List[dict])(
            self._list_location_assessments
        )
        
        # Add convenient endpoint for verified assessments only
        self.router.get("/locations/{location_id}/verified", response_model=List[dict])(
            self._list_verified_assessments
        )

        # Fix the direct assessment endpoint - add it before other assessment_id endpoints
        self.router.get("/{assessment_id}", response_model=LocationSetAssessmentResponse)(self._get_assessment)

        # Add direct access to assessment details
        self.router.get("/{assessment_id}/details", response_model=List[LocationAssessmentResponse])(
            self._get_assessment_details
        )
        # Add POST endpoint for saving assessment details
        self.router.post("/{assessment_id}/details", response_model=LocationAssessmentResponse)(
            self._add_assessment_detail
        )
        # Add endpoint for direct image upload to assessment details
        self.router.post("/{assessment_id}/details/{detail_id}/images", status_code=201)(
            self._upload_assessment_image
        )
        
        # Add endpoint for updating assessment details
        self.router.put("/{assessment_id}/details/{detail_id}", response_model=LocationAssessmentResponse)(
            self._update_assessment_detail
        )
        
        
        # Add new endpoint for complete assessment data
        self.router.get("/{assessment_id}/complete", response_model=dict)(self._get_complete_assessment_data)
        
        # Add endpoint to fix assessments that bypassed submission workflow
        self.router.post("/{assessment_id}/fix", status_code=200)(self._fix_assessment)
        
        # Add endpoint to recalculate overall_score for an assessment
        self.router.post("/{assessment_id}/recalculate", status_code=200)(self._recalculate_assessment)
    ...
    async def _reject(
        ...
        AssessmentService(uow).reject(assessment_id, current, rejection_data.get("reason"))
    ...
    async def _list_criteria(
        self, uow: UnitOfWork = Depends(get_uow), current_user: User = Depends(auth_manager.get_current_user)
    ):
    ...
    async def _get_assessment_set(
        self, set_id: int, uow: UnitOfWork = Depends(get_uow), current_user: User = Depends(auth_manager.get_current_user)
    ):
    ...
    async def _create_location_assessment(
        ...
        service = AssessmentService(uow)
        return service.create_location_assessment(location_id, assessment_data, current_user)

    async def _list_location_assessments(
        self,
        location_id: UUID,
        status: Optional[str] = Query(None, description="Filter by assessment status (e.g., 'verified')"),
        uow: UnitOfWork = Depends(get_uow),
        current_user: User = Depends(auth_manager.get_current_user),
    ):
        logger.info(f"Listing assessments for location {location_id} with status filter: {status}")
        service = AssessmentService(uow)
        ...
        # Convert to response format with additional fields needed for the frontend
        response_data = []
        for assessment in assessments:
            ...
            if assessment.assessor:
                ...
                if assessment.assessor.profile:
                    ...
                    assessor_name = f"{profile.first_name} {profile.surname}".strip()
                if not assessor_name:
                    assessor_name = assessment.assessor.username or assessment.assessor.email
            ...
            if assessment.verifier:
                ...
                if assessment.verifier.profile:
                    ...
                    verifier_name = f"{profile.first_name} {profile.surname}".strip()
                if not verifier_name:
                    verifier_name = assessment.verifier.username or assessment.verifier.email
    ...
    async def _list_verified_assessments(
        ...
        return await self._list_location_assessments(location_id, status="verified", uow=uow, current_user=current_user)
    ...
    async def _get_assessment(
        ...
        response_data = {
            ...
            "location_info": {
                "location_id": assessment.location.location_id if assessment.location else None,
            ...
            "details": [
                {
                    "location_assessment_id": detail.assessment_detail_id,
                    ...
                    "images": [
                        ...
                        } for img in detail.assessment_images
                    ] if detail.assessment_images else []
                }
    ...
    async def _verify_assessment(
        ...
        service = AssessmentService(uow)
        return service.verify_assessment(assessment_id, current_user, verification_data)
    ...
    async def _create_location_assessment_direct(
        self,
        assessment_data: LocationSetAssessmentCreate,
        location_id: UUID = Query(..., description="ID of the location for this assessment"),
        uow: UnitOfWork = Depends(get_uow),
        current_user: User = Depends(auth_manager.get_current_user),
    ):
    ...
    async def _get_complete_assessment_data(
        ...
        # Re-route to the existing _get_assessment logic which aims to do the same
        assessment_response_model = await self._get_assessment(assessment_id=assessment_id, uow=uow, current_user=current_user)
    ...
    async def _update_assessment_detail(
        ...
        logger.info(f"Updating assessment detail {detail_id} for assessment {assessment_id}")
        ...
        return detail_service.update_detail(detail_id, update_payload, current_user)
    ...
    async def _upload_assessment_image(
        ...
        logger.info(f"Uploading image for assessment {assessment_id}, detail {detail_id}")
        ...
        object_key = f"assessment_images/{assessment_id}/{detail_id}/{uuid.uuid4()}.{file_extension}"
    ...
    async def _fix_assessment(
        ...
        try:
            ...
            return {"message": "Assessment fixed successfully", "assessment_id": assessment_id, "overall_score": fixed_assessment.overall_score}
        except Exception as e:
            logger.error(f"Error fixing assessment {assessment_id}: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to fix assessment: {str(e)}")
    ...
    async def _recalculate_assessment(
        ...
        try:
            ...
            logger.error(f"Error recalculating assessment {assessment_id}: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to recalculate assessment: {str(e)}")

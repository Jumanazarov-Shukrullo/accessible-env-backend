...
class RoleRouter:
    ...
    def register_routes(self):
        ...
        self.router.add_api_route(
            "", self.create_role, methods=["POST"], response_model=RoleResponse
        )
        ...
        self.router.add_api_route(
            "/{role_id}/users", self.get_role_users, methods=["GET"]
        )

        # Statistics route
        self.router.add_api_route(
            "/statistics", self.get_statistics, methods=["GET"]
        )

        # Single role routes (must be last)
        self.router.add_api_route("/{role_id}", self.get_role, methods=["GET"])
        self.router.add_api_route(
            "/{role_id}", self.update_role, methods=["PUT"]
        )
        self.router.add_api_route(
            "/{role_id}", self.delete_role, methods=["DELETE"]
        )
    ...
    async def list_roles(
        ...
        end_time = time.time()
        logger.info(
            f"API: list_roles completed in {(end_time - start_time) * 1000:.2f}ms"
        )
        return result
    ...
    async def assign_permission(
        ...
        logger.info(
            f"Assigning permission with id: {permission_id} to role with id: {role_id}")
        return RoleService(db).assign_permission_to_role(
        ...
        try:
            ...
            for user in users:
                ...
                user_data = {
                    ...
                    "last_login_at": (
                        user.security.last_login_at.isoformat()
                        if user.security and user.security.last_login_at
                        else None
                    ),
                }
            ...
            result = {"items": user_list, "total": len(user_list)}
            ...
            raise HTTPException(
                status_code=500, detail="Failed to get role users"
            )
    ...
    async def get_statistics(
        ...
        try:
            ...
            end_time = time.time()
            logger.info(
                f"API: get_statistics completed in {(end_time - start_time) * 1000:.2f}ms"
            )
            return result
        except Exception as e:
    ...
    async def create_permission(
        ...
        try:
            ...
            return {
                ...
                "updated_at": (
                    new_permission.updated_at.isoformat()
                    if new_permission.updated_at
                    else None
                ),
            }
    ...
    async def update_permission(
        ...
        try:
            ...
            if "permission_name" in permission:
                existing_permission.permission_name = permission[
                    "permission_name"
                ]
            ...
            return {
                ...
                "updated_at": (
                    existing_permission.updated_at.isoformat()
                    if existing_permission.updated_at
                    else None
                ),
            }
        except HTTPException:
    ...
    async def list_role_permissions(
        ...
        try:
            ...
            logger.info(
                f"Found {len(permissions)} permissions for role {role_id}"
            )
            logger.info(
                f"Permissions: {[p['permission_name'] for p in permissions]}"
            )
    ...
    async def grant_permission(
        ...
        try:
            ...
            return role_service.assign_permission_to_role(
                role_id, permission_id
            )
        except Exception as e:
            logger.error(f"Error granting permission: {e}")
            raise HTTPException(
                status_code=500, detail="Failed to grant permission"
            )
    ...
    async def revoke_permission(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to revoke permission"
            )

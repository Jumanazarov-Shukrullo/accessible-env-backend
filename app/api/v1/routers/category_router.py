...
class CategoryRouter:
    ...
    def _register(self):
        self.router.post(
            "/", response_model=CategorySchema.Out, status_code=201
        )(self._create_category)
        self.router.get("/", response_model=list[CategorySchema.Out])(
            self._list_tree
        )
        self.router.get("/flat", response_model=list[CategorySchema.Out])(
            self._list_all
        )
        self.router.get("/{category_id}", response_model=CategorySchema.Out)(
            self._get_category
        )
        self.router.put("/{category_id}", response_model=CategorySchema.Out)(
        ...
        self.router.get(
            ...
            response_model=list[CategorySchema.Out],
        )(self._breadcrumb)
    ...
    async def _get_category(
        self, category_id: int, uow: UnitOfWork = Depends(get_uow)
    ):

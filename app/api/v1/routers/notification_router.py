...
class NotificationRouter:
    ...
    def _register(self):
        self.router.post("/", response_model=NotificationResponse, status_code=201)(self._send)
        self.router.get("/unread", response_model=list[NotificationResponse])(self._unread)
        self.router.post("/{notif_id}/read", response_model=NotificationResponse)(self._mark_read)
    ...
    async def _unread(self, uow: UnitOfWork = Depends(get_uow), current: User = Depends(auth_manager.get_current_user)):
        logger.info("Fetching unread notifications for user")
        return NotificationService(uow).unread_for_user(current.user_id)

    async def _mark_read(
        self, notif_id: UUID, uow: UnitOfWork = Depends(get_uow), current: User = Depends(auth_manager.get_current_user)
    ):
        logger.info("Marking notification as read")
        return NotificationService(uow).mark_read(notif_id, current)

    async def _get_notifications(self, _: User = Depends(auth_manager.get_current_user)):
        logger.info("Fetching all notifications")
        return {"notifications": []}

    async def _create_notification(self, current: User = Depends(auth_manager.get_current_user)):
        logger.info("Creating new notification")
        return {"message": "Notification created"}

    async def _delete_notification(self, notification_id: UUID, current: User = Depends(auth_manager.get_current_user)):
